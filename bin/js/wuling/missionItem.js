...
(function (wuling) {
    var missionItem = /** @class */ (function (_super) {
        ...
        missionItem.prototype.miss = function (item, index) {
            var nametext = this.dataSource["nametext"];
            ...
            if (this.missionModule.taskProgress.length > 0) {
                ...
                if (this.missionModule.taskProgress.length > 0) {
                    if (this.missionModule.taskProgress[0][conditions] >= missionNum) {
                        ...
                        this.lingqu.visible = false;
                        this.lingqu.disabled = true;
                        if (this.missionModule.taskProgress[0][conditions] == null) {
                            this.missionModule.taskProgress[0][conditions] = 0;
                        }
                        ;
        ...
        missionItem.prototype.onClick = function () {
            ...
            spAni.addChild(shen);
            shen.playTo(0, 79, new Laya.Handler(this, function () {
                //可以加载对白
                module.missionModule.getInstance().loDialogue = true;
                //  wuling.mission.getInstance().Don();
                //完成任务提示
                if (_this.suoimg.visible == false && _this.dataSource != null) {
                    wuling.Bottom.getInstance().text1.text = _this.dataSource["tuiText"];
                }
                //下次感叹号可以显示
