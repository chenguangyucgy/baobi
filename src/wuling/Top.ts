module wuling{
    export class Top extends ui.topUI{
        // public lv:number = module.Room.getInstance().RoomClass["RoomLevel"];
        //等级
        public lv:number = 1;
        //能量
        public nen:number = module.Room.getInstance().RoomClass["RoomPower"];
        public jin:number = 0;
        constructor(){
            super();
            //定时消失分享提示
            Laya.timer.loop(300000,this,()=>{
                this.shareBtn.visible = true;
                this.WOnShare();
            });

            var headswf:Laya.MovieClip = new Laya.MovieClip;
            this.headKuan.addChild(headswf);
            this.headKuan.scale(0.666,0.666);
            headswf.load("res/swf/touxiang.swf",true);
            headswf.play();
            this.usertext.text =module.Room.getInstance().RoomClass["UserID"];
            // this.headKuan.autoPlay = true;
            // Laya.timer.loop(50, this, this.onHeadKuan);

            this.lv1.index = Math.floor(this.lv/10);
            this.lv2.index = Math.floor(this.lv%10);
            ...
            //宝币动画
            Laya.timer.loop(1, this, this.onStop);

            this.jinbiicon.on(Laya.Event.CLICK, this, this.playjinbi);
            this.gabi();
            this.nens();
            Laya.timer.loop(300000,this,()=>{
            if(this.nen>98){}
        ...
        /**隐藏分享提示点击事件 */
        public WOnShare():void
        {
            Laya.timer.once(5000,this,()=>{
                  Laya.stage.on(Laya.Event.CLICK,this,this.onShare);  
                });
        }
        /**隐藏分享提示 */
        ...
        public zanNum:number =0;//评价总值
        public manyi:number = 0;//满意度
        /**满意度改变*/
        public zan():void
        {
            ...
            // this.yxczt =0;
            // this.dxdrczt =0;
            var RoomLv:number = module.Room.getInstance().RoomClass["RoomLevel"];
            ...
            //     }
            // }
            var lastNum:number = this.zanNum;
            ...
            var nowNum:number = lastNum - this.zanNum;
            //评估值动画
            if(nowNum>0){
            ...
            nowNum = Math.abs(nowNum);
                ...
                Laya.Tween.to(this.jian, {y: 90, alpha: 1}, 500, null, Laya.Handler.create(this, ()=>{
                    Laya.Tween.to(this.jian, {y: 105, alpha:0}, 300);
                }));
            
            var zanString:string = ""+this.zanNum+"";
            // var zanString:string = "50550";
            this.zanNum = Number(zanString);
            for(var g:number = 1; g < 5; g++)
            {
        ...
        // private onHeadKuan():void{
        //     if(this.headKuan.index == 29){
        //         Laya.timer.clear(this, this.onHeadKuan);
        //         this.headKuan.autoPlay = false;
        //         Laya.timer.once(1500, this, this.onGo)
        //     }
        // }
        // private onGo():void{
        //     this.headKuan.index = 0;
        //     this.headKuan.autoPlay = true;
        //     Laya.timer.loop(50, this, this.onHeadKuan);
        // }

        private static _instance:Top;
		public static getInstance():Top
		{
