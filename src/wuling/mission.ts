...
module wuling{
	export class mission extends ui.missionUI{
		...
		private static _instance:mission;
		public static getInstance():mission
		{
		...
		private renderHandler(item:missionItem, index:number):void
		{
			item.miss(item, index);
		}
		private renderHandler2(item:achievement, index:number):void
		{
			item.miss(item, index);
		}
		private onSelect(event:Laya.Event):void
		{
