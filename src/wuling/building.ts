...
*/
module wuling{
	export class building extends ui.buildingUI{
		private buildingModules:module.buildingModule = module.buildingModule.getInstance();

		private arr2:Array<Object> = this.buildingModules.buildingChoose(this.buildingModules.buildingChooseData1);
		private arr1:Array<Object> = this.buildingModules.buildingChoose(this.buildingModules.buildingChooseData2);
		private arr31:Array<Object> =[
		...
		constructor(){
			...
			this.box1.on(Laya.Event.CLICK,this,this.switchClass);
			this.box2.on(Laya.Event.CLICK,this,this.switchClass);
			this.box3.on(Laya.Event.CLICK,this,this.switchClass);
			...
			if(wuling.map.getInstance().guide==true)
			{
				...
				if(module.mapModule.getInstance().buildinghS==2)
				{
					Laya.timer.once(100,this,()=>{
						this.list.scrollBar.value =1402;
					});
				}
			}
			//隐藏开始任务提示
		...
		private renderHandler(item:buildingItem,index:number):void
		{
			item.show(index);
		...
		//切换效果
		private switchClass(event:Laya.Event):void
		{
			switch (event.currentTarget) {
				case this.box1:
				    this.list.array = this.arr1
				...
				case this.box3:
				    ...
				    if(RoomClass["RoomLevel"]>=0&&RoomClass["RoomLevel"]<4){this.list.array = this.arr31;}
					if(RoomClass["RoomLevel"]>=4&&RoomClass["RoomLevel"]<9){this.list.array = this.arr32;}
					if(RoomClass["RoomLevel"]>=9&&RoomClass["RoomLevel"]<14){this.list.array = this.arr33;}
					if(RoomClass["RoomLevel"]>=14){this.list.array = this.arr34;}
